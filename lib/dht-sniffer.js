...
class DHTSniffer extends events_1.EventEmitter {
    ...
    fetchMetaData(infoHash, peer, mode = false) {
        const _this = this;
        if (mode === false) {
            if (!peer || (peer.address === undefined || peer.port === undefined))
                return false;
        }
        if (peer) {
    ...
    getSizes() {
        let fetchings = Object.keys(this.metadataFetchingDict);
        console.log(fetchings.length, this.metadataWaitingQueues.length, this.fetchdTuple.keyMap.size, this.fetchdInfoHash.keyMap.size, this.rpc.pending.length);
    }
