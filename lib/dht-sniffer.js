(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "crypto", "./dht/dht", "./metadata/metadata-helper", "events", "./utils", "lru-cache", "./utils/error-handler", "./utils/error-monitor"], factory);
    }
    ...
    class DHTSniffer extends events_1.EventEmitter {
        constructor(options) {
            ...
            this.fetchedTuple = new lru_cache_1.LRUCache({ max: this._options.fetchedTupleSize, ttl: 3 * 60 * 60 * 1000 });
            this.fetchedInfoHash = new lru_cache_1.LRUCache({ max: this._options.fetchedInfoHashSize, ttl: 72 * 60 * 60 * 1000 });
            this.findNodeCache = new lru_cache_1.LRUCache({ max: this._options.findNodeCacheSize, ttl: 24 * 60 * 60 * 1000, updateAgeOnHas: true });
            this.latestCalledPeers = new lru_cache_1.LRUCache({ max: 1000, ttl: 5 * 60 * 1000 });
            this.usefulPeers = new lru_cache_1.LRUCache({ max: 5000 });
            this.metadataFetchingCache = new lru_cache_1.LRUCache({ max: 1000, ttl: 20 * 1000 });
            ...
            this.errorMonitor = new error_monitor_1.ErrorMonitor(this.errorHandler, {
        ...
        start() {
            ...
            this.dht.on('warning', err => {
                const warning = new error_handler_1.DHTError(`DHT warning: ${err.message || err}`, { operation: 'dht_operation' }, err instanceof Error ? err : new Error(String(err)), true);
                const warningError = new error_handler_1.DHTError(`DHT warning: ${warning}`, {
                    operation: 'dht_warning',
                    component: 'dht',
                    severity: error_handler_1.ErrorSeverity.WARNING
                });
                this.errorHandler.handleError(warningError);
                _this.emit('warning', warning);
            });
            this.dht.on('error', err => {
                const error = new error_handler_1.DHTError(`DHT error: ${err.message || err}`, { operation: 'dht_operation' }, err instanceof Error ? err : new Error(String(err)), false);
        ...
        fetchMetaData(infoHash, peer, mode = false) {
            ...
            if (mode) {
                this.dht.lookup(infoHash, function (err, totalNodes) {
                    if (err) {
                        const error = new error_handler_1.DHTError(`DHT lookup failed for ${infoHash.toString('hex')}: ${err.message || err}`, { operation: 'dht_lookup', infoHash: infoHash.toString('hex') }, err instanceof Error ? err : new Error(String(err)));
