    ...
    class PeerManager extends base_manager_1.BaseManager {
        ...
        exportUsefulPeers() {
            ...
            for (const [_key, value] of usefulPeers) {
                if (value.peer && value.peer.host && value.peer.port) {
                    peers.push(Object.assign({}, value.peer));
                }
            }
            return peers;
        }
        ...
        addPeer(peerInfo) {
            ...
            if (!peerInfo.peer.host || !peerInfo.peer.port) {
                this.handleError('addPeer', new error_1.ValidationError('Peer address or port is undefined', { peer: peerInfo.peer }), { peer: peerInfo.peer, errorType: error_1.ErrorType.VALIDATION });
                return;
            }
            try {
        ...
        exportNodes() {
            return this.nodes.filter(node => node && node.host && node.port);
        }
        importNodes(nodes) {
            ...
            try {
                for (const node of nodes) {
                    if (!node || !node.host || !node.port) {
                        this.handleError('importNodes', new error_1.ValidationError('Node address or port is undefined', { node }), { node, errorType: error_1.ErrorType.VALIDATION });
                        continue;
                    }
                    this.addNode(node);
                }
        ...
        exportPeers() {
            const allPeers = this.cacheManager.getAllPeers();
            return allPeers.filter(peer => peer && peer.host && peer.port);
        }
        importPeers(peers) {
