...
(function (factory) {
    ...
    class DHTManager extends base_manager_1.BaseManager {
        constructor(config, errorHandler, peerManager, cacheManager) {
            super(config, errorHandler);
            this.lastRefreshTime = 0;
            this.metadataWaitingQueues = [];
            this.findNodeQueue = [];
            this.isProcessingFindNodeQueue = false;
            this.lastFindNodeTime = 0;
            this.findNodeInterval = 50;
        ...
        setupEventListeners() {
            ...
            this.dht.on('node', (node) => {
                ...
                if (node && node.host && node.port) {
                    ...
                    if (!((_a = this.cacheManager.findNodeCache) === null || _a === void 0 ? void 0 : _a.get(nodeKey)) &&
                    ...
                    shuffledNodes.forEach(node => {
                        ...
                        const shouldCallFindNode = nodes.length < 5 ||
                            (this.peerManager.shouldCallFindNode(node) &&
                                nodes.length < 400 &&
                                Math.random() > (((_b = (_a = this.dht._rpc) === null || _a === void 0 ? void 0 : _a.pending) === null || _b === void 0 ? void 0 : _b.length) || 0) / 12);
        ...
        processFindNodeQueue() {
            ...
            const processNext = () => {
                ...
                if (timeSinceLastFindNode >= this.findNodeInterval) {
                    const request = this.findNodeQueue.shift();
                    if (request) {
        ...
        executeFindNodeRequest(node, nodeId) {
            if (!this.dht || !this.isDHTRunning())
                return;
            try {
                if (!this.peerManager.shouldCallFindNode(node))
                    return;
                ...
                this.dht._rpc.query(node, message, (err, reply) => {
                    try {
                        if (node && node.id && this.dht._rpc.nodes.get(node.id) && (0, utils_1.isNodeId)(node.id, 20)) {
                ...
                const networkError = new error_1.NetworkError(`Failed to find node: ${error instanceof Error ? error.message : String(error)}`, { operation: 'dht_find_node', node, cause: error instanceof Error ? error : new Error(String(error)) }, true);
