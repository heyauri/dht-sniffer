(function (factory) {
    ...
    function validateConfig(config) {
        if (config.address !== undefined && (typeof config.address !== 'string' || config.address.trim() === '')) {
            throw new error_1.ValidationError('address must be a non-empty string if provided', { field: 'address', value: config.address });
        }
        if (config.port !== undefined) {
            if (typeof config.port !== 'number' || config.port < 1 || config.port > 65535) {
                throw new error_1.ValidationError('port must be a number between 1 and 65535', { field: 'port', value: config.port });
            }
        }
        if (typeof config.nodesMaxSize !== 'number' || config.nodesMaxSize < 1) {
            throw new error_1.ValidationError('nodesMaxSize must be a positive number', { field: 'nodesMaxSize', value: config.nodesMaxSize });
        }
        if (typeof config.refreshPeriod !== 'number' || config.refreshPeriod < 1000) {
            throw new error_1.ValidationError('refreshPeriod must be at least 1000ms', { field: 'refreshPeriod', value: config.refreshPeriod });
        }
        if (typeof config.announcePeriod !== 'number' || config.announcePeriod < 1000) {
            throw new error_1.ValidationError('announcePeriod must be at least 1000ms', { field: 'announcePeriod', value: config.announcePeriod });
        }
        if (config.bootstrap !== undefined) {
            if (typeof config.bootstrap !== 'boolean' && !Array.isArray(config.bootstrap)) {
                throw new error_1.ValidationError('bootstrap must be a boolean or string array', { field: 'bootstrap', value: config.bootstrap });
            }
            if (Array.isArray(config.bootstrap)) {
                for (let i = 0; i < config.bootstrap.length; i++) {
                    const node = config.bootstrap[i];
                    if (typeof node !== 'string') {
                        throw new error_1.ValidationError(`bootstrap[${i}] must be a string`, { field: `bootstrap[${i}]`, value: node });
                    }
                }
            }
        }
    ...
    class DHTManager extends base_manager_1.BaseManager {
        ...
        findNode(peerOrTarget, nodeId) {
            ...
            try {
                ...
                const networkError = new error_1.NetworkError(`Failed to find node: ${error instanceof Error ? error.message : String(error)}`, { operation: 'dht_find_node', peer: peerOrTarget, cause: error instanceof Error ? error : new Error(String(error)) }, true);
        ...
        executeWithRetry(operationKey, operation, operationName, context) {
            ...
            const attempt = (attemptNumber) => {
                try {
                    ...
                    if (currentRetries < maxRetries) {
                        ...
                        }, retryDelay * Math.pow(2, currentRetries));
                        this.emit('retry', {
                        ...
                        const networkError = new error_1.NetworkError(`Failed after ${maxRetries} retries: ${error instanceof Error ? error.message : String(error)}`, Object.assign(Object.assign({ operation: operationName }, context), { cause: error instanceof Error ? error : new Error(String(error)) }));
